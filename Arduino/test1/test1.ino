...
void inverseKinematics(float x_EE, float y_EE, float z_EE) {
  
  // Calculate q1
  q1 = atan2(y_EE, x_EE);

  // Calculate position of joint #4
  float x_4 = x_EE - (L4 * cos(q1));
  float y_4 = y_EE - (L4 * sin(q1));
  float z_4 = z_EE;
  ...
  // Calculate q3
  q3 = - (pi - acos((L2 * L2 + L3 * L3 - v_side * v_side) / (2 * L2 * L3)));
...
std_msgs::Bool is_harvest;
ros::Publisher harvest_pub("harvest_topic", &is_harvest);
...
void loop() {
  ...
  switch (currentState) {
    ...
    case MOVE_TO_ORIGIN:
      ...
      break;
    case MOVE_TO_GOAL:
      inverseKinematics(x_EE, y_EE, z_EE);
      Serial.println(q1);
      Serial.println(q2);
      Serial.println(q3);
      while (!isAtGoal) {
    ...
    case CLOSE_GRIP:
      ...
      break;
    case MOVE_TO_BOX:
      inverseKinematics(x_box, y_box, z_box);
      if (currentState == MOVE_TO_BOX) {
        while (!isAtGoal) {
          moveToGoal(q1, q2, q3);
        }
    ...
    case MOVE_TO_LIMITS:
      ...
      if (xLimitFlag && yLimitFlag && zLimitFlag) {
        ...
        q2 = 140;
        q3 = -180;
        is_harvest.data = true;
        harvest_pub.publish(&is_harvest);
        Serial.println("done limit");
        break;
      }
      break;
...
void moveToGoal(float q1, float q2, float q3) {
  unsigned long currentMillis = millis();
  ...
  if (currentMillis - previousStepTime >= stepInterval) {
    ...
    float xGoalPos = stepper1.currentPosition() + ((q1 - q1_c) / 1.8 * 5);
    float yGoalPos = stepper2.currentPosition() + (-(q2 - q2_c) / 1.8 * 3);
    float zGoalPos = stepper3.currentPosition() - ((q3 - q3_c) / 1.8 * 3);
  ...
  Serial.print("Stepper1 Distance to go: ");
  Serial.println(stepper1.distanceToGo());
  Serial.print("Stepper2 Distance to go: ");
  Serial.println(stepper2.distanceToGo());
  Serial.print("Stepper3 Distance to go: ");
